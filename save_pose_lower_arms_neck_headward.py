...
LANDMARK_GROUP = {
    ...
    'head': {'HEAD_TOP'}  # Added head group for the new head top point
}
...
def calculate_head_top(results):
    """Calculate head top point from available head landmarks"""
    # Get available head landmarks
    nose = safe_get(results, mp_pose.PoseLandmark.NOSE)
    left_eye = safe_get(results, mp_pose.PoseLandmark.LEFT_EYE)
    right_eye = safe_get(results, mp_pose.PoseLandmark.RIGHT_EYE)
    left_eye_inner = safe_get(results, mp_pose.PoseLandmark.LEFT_EYE_INNER)
    right_eye_inner = safe_get(results, mp_pose.PoseLandmark.RIGHT_EYE_INNER)
    left_ear = safe_get(results, mp_pose.PoseLandmark.LEFT_EAR)
    right_ear = safe_get(results, mp_pose.PoseLandmark.RIGHT_EAR)
    
    # Collect available head points
    head_points = []
    if nose:
        head_points.append((nose.x, nose.y, getattr(nose, "z", 0.0)))
    if left_eye:
        head_points.append((left_eye.x, left_eye.y, getattr(left_eye, "z", 0.0)))
    if right_eye:
        head_points.append((right_eye.x, right_eye.y, getattr(right_eye, "z", 0.0)))
    if left_eye_inner:
        head_points.append((left_eye_inner.x, left_eye_inner.y, getattr(left_eye_inner, "z", 0.0)))
    if right_eye_inner:
        head_points.append((right_eye_inner.x, right_eye_inner.y, getattr(right_eye_inner, "z", 0.0)))
    if left_ear:
        head_points.append((left_ear.x, left_ear.y, getattr(left_ear, "z", 0.0)))
    if right_ear:
        head_points.append((right_ear.x, right_ear.y, getattr(right_ear, "z", 0.0)))
    
    if not head_points:
        return None, None, None
    
    # Calculate center of head points
    avg_x = sum(p[0] for p in head_points) / len(head_points)
    avg_y = sum(p[1] for p in head_points) / len(head_points)
    avg_z = sum(p[2] for p in head_points) / len(head_points)
    
    # Find the highest point (smallest y value since y increases downward)
    min_y = min(p[1] for p in head_points)
    
    # Calculate head top: move upward from center by the distance to highest point
    head_height_offset = avg_y - min_y
    head_top_y = min_y - head_height_offset * 0.3  # Add 30% more height above the highest point
    
    return avg_x, head_top_y, avg_z
        ...
        while True:
            ...
            if do_process:
                ...
                if results.pose_landmarks:
                    ...
                    # calculate head top point
                    head_top_x, head_top_y, head_top_z = calculate_head_top(results)
                    ...
                    # draw head top if available
                    if head_top_x is not None:
                        head_top_px = (int(head_top_x * width + 0.5), int(head_top_y * height + 0.5))
                        cv2.circle(annotated, head_top_px, 5, (255,255,0), -1)  # Yellow color for head top
                        cv2.putText(annotated, "head_top", (head_top_px[0]+4, head_top_px[1]-4),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,0), 1)

                    # draw connection between neck and head top
                    if neck_x is not None and head_top_x is not None:
                        neck_px = (int(neck_x * width + 0.5), int(neck_y * height + 0.5))
                        head_top_px = (int(head_top_x * width + 0.5), int(head_top_y * height + 0.5))
                        cv2.line(annotated, neck_px, head_top_px, (255,128,0), 3)  # Orange line for neck-head connection
            ...
            if SHOW_WINDOW:
                ...
                cv2.imshow("Pose - Lower body + arms + neck + head_top", display)
