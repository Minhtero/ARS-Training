...
# --------- CONFIGURATION ----------
CAMERA_INDEX = 0
OUTPUT_VIDEO = r"mediasquat_lower_arms_neck_headward.mp4"
SAVE_LANDMARKS = True
LANDMARK_CSV = r"landmarks_lower_arms_neck_headward.csv"
...
with mp_pose.Pose(static_image_mode=False,
    ...
    try:
        while True:
            ...
            head_x = head_y = head_z = None  # new virtual landmark: top of head
            ...
            if do_process:
                ...
                if results.pose_landmarks:
                    ...
                    if l_sh and r_sh and nose:
                        ...
                        # neck = 60% shoulders midpoint + 40% nose  => moves neck toward the head
                        neck_x = 0.6*mid_sh_x + 0.4*float(nose.x)
                        neck_y = 0.6*mid_sh_y + 0.4*float(nose.y)
                        neck_z = 0.6*mid_sh_z + 0.4*float(getattr(nose,"z",0.0))
                        # head top by extending vector neck->nose by additional 30%
                        dx = float(nose.x) - neck_x
                        dy = float(nose.y) - neck_y
                        dz = float(getattr(nose,"z",0.0)) - neck_z
                        head_x = float(nose.x) + 0.3*dx
                        head_y = float(nose.y) + 0.3*dy
                        head_z = float(getattr(nose,"z",0.0)) + 0.3*dz
                    elif l_sh and r_sh:
                    ...
                    # draw head top and connection to neck
                    if head_x is not None:
                        head_px = (int(head_x * width + 0.5), int(head_y * height + 0.5))
                        cv2.circle(annotated, head_px, 5, (255,0,255), -1)
                        cv2.putText(annotated, "head", (head_px[0]+4, head_px[1]-4),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,0,255), 1)
                        if neck_x is not None:
                            cv2.line(annotated, neck_px, head_px, (255,0,255), 2)
                    ...
                    if SAVE_LANDMARKS and csv_writer is not None:
                        ...
                        # write HEAD_TOP row
                        if head_x is None:
                            csv_writer.writerow([frame_idx, "HEAD_TOP", -1, "head", float('nan'), float('nan'), float('nan'), '', ''])
                        else:
                            head_px = (int(head_x * width + 0.5), int(head_y * height + 0.5))
                            csv_writer.writerow([frame_idx, "HEAD_TOP", -1, "head", head_x, head_y, head_z, head_px[0], head_px[1]])

            # FPS overlay every 10 frames
            if frame_idx % 10 == 0:
            ...
            if SHOW_WINDOW:
                ...
                cv2.imshow("Pose - Lower body + arms + neck + head", display)
...
if SAVE_LANDMARKS:
    print("CSV saved to:", LANDMARK_CSV)