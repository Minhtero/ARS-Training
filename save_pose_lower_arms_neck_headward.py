        ...
        while True:
            ...
            if do_process:
                ...
                if results.pose_landmarks:
                    ...
                    l_ear = safe_get(results, mp_pose.PoseLandmark.LEFT_EAR)
                    r_ear = safe_get(results, mp_pose.PoseLandmark.RIGHT_EAR)
                    ...
                    # compute head top point above nose along shoulder->nose direction
                    if nose and l_sh and r_sh:
                        mid_sh_x = (l_sh.x + r_sh.x) / 2.0
                        mid_sh_y = (l_sh.y + r_sh.y) / 2.0
                        mid_sh_z = (getattr(l_sh,"z",0.0)+getattr(r_sh,"z",0.0)) / 2.0
                        v_x = float(nose.x) - mid_sh_x
                        v_y = float(nose.y) - mid_sh_y
                        v_z = float(getattr(nose,'z',0.0)) - mid_sh_z
                        scale = 0.35
                        head_top_x = float(nose.x) + scale * v_x
                        head_top_y = float(nose.y) + scale * v_y
                        head_top_z = float(getattr(nose,'z',0.0)) + scale * v_z
                    elif nose and l_hip and r_hip:
                        mid_hip_x = (l_hip.x + r_hip.x) / 2.0
                        mid_hip_y = (l_hip.y + r_hip.y) / 2.0
                        mid_hip_z = (getattr(l_hip,'z',0.0) + getattr(r_hip,'z',0.0)) / 2.0
                        v_x = float(nose.x) - mid_hip_x
                        v_y = float(nose.y) - mid_hip_y
                        v_z = float(getattr(nose,'z',0.0)) - mid_hip_z
                        scale = 0.20
                        head_top_x = float(nose.x) + scale * v_x
                        head_top_y = float(nose.y) + scale * v_y
                        head_top_z = float(getattr(nose,'z',0.0)) + scale * v_z
                    ...
                    # draw head top and connect to neck if available
                    if head_top_x is not None:
                        head_top_px = (int(head_top_x * width + 0.5), int(head_top_y * height + 0.5))
                        cv2.circle(annotated, head_top_px, 5, (255,0,255), -1)
                        cv2.putText(annotated, "head_top", (head_top_px[0]+4, head_top_px[1]-4),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,0,255), 1)
                        if neck_px is not None:
                            cv2.line(annotated, neck_px, head_top_px, (255,0,255), 2)
